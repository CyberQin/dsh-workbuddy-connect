	...
	factory: (require) => {
		...
		*/
		...
		function writeSeen(marks) {
			try {
				window.localStorage.setItem(SEEN_STORAGE_KEY, JSON.stringify([...marks].slice(-200)));
			} catch {}
		}
		const wrapperStyle = {
			display: "inline-flex",
		...
		*/
		...
		function ModelProbe({ model, label, t }) {
			...
			const seen = (0, react.useRef)(void 0);
			const selfStarted = (0, react.useRef)(void 0);
			...
			(0, react.useEffect)(() => {
				...
				if (started === void 0 || started.id !== model || result.probedAt < started.atMs) {
					seen.current.add(mark);
					writeSeen(seen.current);
					return;
				}
				selfStarted.current = void 0;
				setNote({
					result,
					mark
				});
		...
		*/
		function ContextTable({ models, t }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				}), known.map((model) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					style: quotaLabelStyle,
