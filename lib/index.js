	...
	return async (req, res) => {
		...
		try {
			...
			if (action.action === "set-model-visibility") {
				...
				json(res, 200, await deps.setModelVisibility(action.model, action.visible === true));
				return;
			}
...
function createVariantRuntime(config, variant, current, identityOf, accountOf) {
...
function apply(ctx, config) {
	...
	*/
	...
	const runtimes = WORKBUDDY_VARIANTS.map((variant) => createVariantRuntime(config, variant, () => current(), (id) => lastIdentities.get(id), (id) => lastAccounts.get(id)));
