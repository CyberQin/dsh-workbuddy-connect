...
async function workBuddyWebStatus(deps) {
	...
	const modelsField = deps.models().map((model) => {
		...
		return {
			...
			...typeof model.defaultContextWindow === "number" && model.defaultContextWindow > 0 ? { defaultContextWindow: model.defaultContextWindow } : {},
...
const MAXIMUM_CONTEXT_WINDOW_FIELD = z.boolean().default(false).description("Use the largest context window declared by WorkBuddy AI when alternatives are available");
