...
async function resolveChatIdentity(region, options = {}) {
	...
	const identity = region === "global" ? await resolveGlobalIdentity(options) : await resolveCnIdentity(options);
	...
	if (installed !== void 0 && validAppVersion(installed.version)) {
		...
		try {
			await writeFileAtomic(savedPath, `${JSON.stringify({
				...
				...identity.cliVersion === void 0 ? {} : { cliVersion: identity.cliVersion },
				observedAt: Date.now()
			}, null, 2)}\n`, {
				mode: 384,
				dirMode: 448
	...
	try {
		...
		if (typeof saved === "object" && saved !== null && !Array.isArray(saved)) {
			...
			if (validAppVersion(document["version"])) {
				const cliVersion = validCliVersion(document["cliVersion"]) ? document["cliVersion"] : void 0;
				return {
...
//#endregion
export { parseModelCatalog as A, readCliVersion as B, desktopAuthCandidatesFor as C, classifyUpstreamError as D, WorkBuddyUpstreamClient as E, probeModel as F, installedAppVersion as G, validCliVersion as H, randomSentinel as I, validAppVersion as J, readBundleVersion as K, CN_APP_VERSION_FILENAME as L, prepareInternationalChatBody as M, regionOf as N, modelWithCurrentPromotion as O, PROBE_EFFORT_CANDIDATES as P, FALLBACK_CN_APP_VERSION as R, defaultDesktopAuthPath as S, workbuddyOwnAuthPath as T, WORKBUDDY_APP_VERSION_FILENAME as U, resolveChatIdentity as V, appUserAgent as W, WorkBuddyCatalog as _, WORKBUDDY_HOST_HEARTBEAT_FILENAME as a, WorkBuddyCredentialStore as b, processStartTimeMs as c, writeHostHeartbeat as d, WORKBUDDY_CONNECT_VERSION as f, FALLBACK_WORKBUDDY_MODELS as g, FALLBACK_WORKBUDDY_AI_MODELS as h, variantFor as i, prepareChatBody as j, normalizeCredits as k, readHostHeartbeat as l, WORKBUDDY_STATUS_PATH as m, CN_VARIANT as n, clearHostHeartbeat as o, WORKBUDDY_PROBE_PATH as p, resolveAppVersion as q, WORKBUDDY_VARIANTS as r, isHeartbeatProcessAlive as s, AI_VARIANT as t, workbuddyHostHeartbeatPath as u, WORKBUDDY_AUTH_FILENAME as v, parseWorkBuddyAuth as w, defaultDesktopAuthCandidates as x, WORKBUDDY_AUTH_FILE_ENV as y, chatUserAgent as z };
