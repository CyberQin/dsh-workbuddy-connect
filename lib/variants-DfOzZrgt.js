...
var WorkBuddyUpstreamClient = class {
	...
	*/
	async fetchModels(credential) {
		...
		const appVersion = international ? await resolveAppVersion() : void 0;
		...
		const envelope = await readEnvelope(response);
		if (!response.ok || envelope.code !== 0) throw envelopeError(response.status, envelope);
		const models = parseModelCatalog(isObject(envelope.data) ? envelope.data : {}, international);
		this.lastCatalog = {
...
function modelWithCurrentPromotion(model, now = Date.now()) {
	...
	if (promotion === void 0) return model;
