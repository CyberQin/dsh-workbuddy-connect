...
**v0.5.0 起支持** WorkBuddy AI 国际版。安装并登录 WorkBuddy AI 桌面 App 后，DSH 会出现独立的「WorkBuddy AI」模型分组和「DSH WorkBuddy AI Connect」设置卡片。

- 国际版与国内版使用不同的登录凭据、模型目录、积分和设置；两版可同时使用，互不覆盖。
- 只登录国际版时只显示国际版分组；国际版没有有效登录凭据时，不显示该分组，避免出现选中后必然失败的模型。
- 国际版模型目录由 App 的 User-Agent 获取，因此模型、价格、促销、上下文窗口和推理档位以该账号当前目录为准。刷新失败时依次使用该账号上次成功保存的目录和内置兜底目录，卡片会标明来源。
- macOS 已完成国际版真实 shim 验证。Windows、WSL 和 Linux 暂时使用已保存或内置的 App 版本生成 User-Agent，详见[已知限制](#已知限制)。
...
|---|---|---|
| **0.5.0** | `0.1.5-rc.1` 及以上 | 支持 WorkBuddy 与 WorkBuddy AI；待 App 内置核心跟进 |
| **0.3.2 – 0.4.0** | `0.1.5-rc.1` 及以上 | 仅支持 WorkBuddy 国内版；待 App 内置核心跟进 |
