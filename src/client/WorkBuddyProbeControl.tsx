...
function ModelProbe({ model, label, t }: { model: string; label: string } & WorkBuddyPluginCardInjected) {
  ...
  const dismissed = useRef<string | undefined>()
  ...
  // A selection change or a completed sweep must not strand an open bubble.
  useEffect(() => { setConfirming(false); dismissed.current = undefined }, [model])
  useEffect(() => {
    if (result === undefined) {
      setNote(undefined)
      return
